...
$phone = test_input($_POST["phone"]);
    if (!preg_match('/^[0-9]{11}+$/', $phone)) {
     echo "<script>alert('Error! Contact number not valid'); window.location='index.php'</script>";; 
    }
else{	
...
$result=mysqli_query($conn,"select * from messages WHERE email='$email' ") or die (mySQL_error());
					...
					if ($conn->query($sql) === TRUE) {
                   		"<script>alert('Pasok'); window.location='contact.php'</script>";
						}else{
...
echo "<script>alert('Wrong email'); window.location='contact.php'</script>";
