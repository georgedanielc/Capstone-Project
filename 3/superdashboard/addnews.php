                    ...
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
							...
							<form method="POST" enctype="multipart/form-data" action="">
									...
									<label class="control-label .col-lg-2" for="first-name">News name
                                    </label>
									 <div class="form-padding-correction">
							         <input type="text" name="nname" class="form-control"  placeholder="Enter Name" required>
								    </div>
									</div>
																     	<div class="form-padding">
...
$nname = $_POST['nname'];
$info = $_POST['info'];
                    $result=mysqli_query($conn,"select * from news WHERE nname='$nname' ") or die (mySQL_error());
					$row=mysqli_num_rows($result);
					if ($row > 0)
					{
				    echo "<script>alert('News Already active!'); window.location='addbanner.php'</script>";

					}
					else
					{		
                   
						$sql="insert into news (nname, info, nimage) values ('$nname','$info','$book_image')";
						if ($conn->query($sql) === TRUE) { 
                   		echo "<script>alert('News successfully added!'); window.location='news.php'</script>";
                        } else {
							echo "<script>alert('News sad!'); window.location='addnews.php'</script>";
                        }
