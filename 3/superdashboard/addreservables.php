        ...
        <div id="page-wrapper">
                        ...
                        <div class="panel-body">
							...
							<form method="POST" enctype="multipart/form-data" action="">
									...
									<label class="control-label .col-lg-2" for="first-name">Item Id
							         ...
							         <input type="text" name="item_id" class="form-control" placeholder="Enter Id" >
								    </div>
									</div>
									...
									<label class="control-label .col-lg-2" for="first-name">Information
									 ...
									 <div class="form-padding-correction">
							         <input type="text" name="itemdesc" class="form-control" placeholder="Enter Information" >
								    </div>
									</div>
			...
			$error = $_FILES["image"] ["error"];
				...
				if($size > 10000000000000) //conditions for the file
					...
					move_uploaded_file($_FILES["image"]["tmp_name"],"reservables/" . $_FILES["image"]["name"]);	
...
$quantity = $_POST['quantity'];
					...
					if ($row > 0)
					{
				    echo "<script>alert('Item Already active!'); window.location='addresservables.php'</script>";

					}
					else
					{		
                   
						$sql="insert into items (item_id, rname, itemdesc, requirements, quantity, iimage,type) values ('$item_id','$rname','$itemdesc', '$requirements', '$quantity', '$book_image','reservables')";
						if ($conn->query($sql) === TRUE) { 
                   		echo "<script>alert('Item successfully added!'); window.location='reservables.php'</script>";
                        } else {
							echo "<script>alert('News sad!'); window.location='addreservables.php'</script>";
                        }
